...
HANDLE WINAPI CreateEvent(void *pDummySec, bool bManualReset, bool bInitialState, char *szDummyName)
{
  ...
  pHandle->m_hCond = XBMC_CreateCond();
  pHandle->m_hMutex = XBMC_CreateMutex();
...
bool WINAPI SetEvent(HANDLE hEvent)
{
  ...
  if (hEvent->m_bManualEvent == true)
    XBMC_CondBroadcast(hEvent->m_hCond);
  else
    XBMC_CondSignal(hEvent->m_hCond);
